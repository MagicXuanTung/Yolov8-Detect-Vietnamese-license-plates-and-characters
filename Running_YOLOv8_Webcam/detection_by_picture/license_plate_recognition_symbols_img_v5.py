    ...
    for i, r in enumerate(results):
        ...
        for j, box in enumerate(boxes):
            ...
            if classNames[cls] == "license_plate":
                ...
                cv2.namedWindow(
                    f"License Plate Color {cropped_images_count + 1}", cv2.WINDOW_AUTOSIZE)
                cv2.resizeWindow(
                    f"License Plate Color {cropped_images_count + 1}", 640, 360)
                cv2.imshow(
                    f"License Plate Color {cropped_images_count + 1}", cropped_img)
                ...
                cv2.namedWindow(
                    f"Preprocessed Image {cropped_images_count + 1}", cv2.WINDOW_AUTOSIZE)
                cv2.resizeWindow(
                    f"Preprocessed Image {cropped_images_count + 1}", 640, 360)
                cv2.imshow(
                    f"Preprocessed Image {cropped_images_count + 1}", cropped_max_contrast)
                ...
                # Print the license plate text to the console
                print(f"License Plate black and white:\n{license_plate_text}")
                ...
                cv2.imwrite(cropped_output_filename_bw, cropped_max_contrast)
