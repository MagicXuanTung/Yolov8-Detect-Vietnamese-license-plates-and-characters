    ...
    for i, r in enumerate(results):
        ...
        for j, box in enumerate(boxes):
            ...
            if classNames[cls] == "license_plate":
                ...
                # Xác định tâm của vùng cần xoay (center của ảnh)
                # center = ((crop_x2 - crop_x1) // 2, (crop_y2 - crop_y1) // 2)

                # # Tạo ma trận biến đổi affine để xoay ảnh
                # angle = 10  # Đặt góc xoay (đơn vị: độ)
                # rotation_matrix = cv2.getRotationMatrix2D(center, angle, 1.0)

                # # Thực hiện xoay ảnh
                # rotated_img = cv2.warpAffine(
                #     cropped_img, rotation_matrix, (crop_x2 - crop_x1, crop_y2 - crop_y1))
